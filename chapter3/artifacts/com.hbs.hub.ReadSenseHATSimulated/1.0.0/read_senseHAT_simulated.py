...
"""read_senseHAT_simulated.py: simulate sensor values and publish them on IPC

   Generates simulated sensor values as if a SenseHAT expansion module
   was installed on a Raspberry Pi. Publish measured values
   to a local topic using IoT Greengrass IPC. Use this component
   when deploying book examples without a SenseHAT device.
"""
...
# constants for simulating random analog signals
TEMPERATURE_MU = 18
TEMPERATURE_SIGMA = 9
HUMIDITY_MU = 55
HUMIDITY_SIGMA = 20
...
class SenseHat:
  ...
  Defines two of the same methods as the actual SenseHAT library.
  Methods generate numbers randomly from a gaussian distribution.
  """
  @staticmethod
  def get_temperature():
    return random.gauss(TEMPERATURE_MU, TEMPERATURE_SIGMA)
  
  @staticmethod
  def get_humidity():
    return random.gauss(HUMIDITY_MU, HUMIDITY_SIGMA)
